...
class ThreatAnalysisAutoTasks:
    ...
    def trust_boundary_identification_task(self, agent):
        return Task(description=dedent("""
            ...
            identify the trust zones for each node in the system.
            </task>
        ...
        """).format(tip_section = self.__tip_section()),
            ...
            - Append trust zone rules above to the end of the output
            """
            ))
    
    def threat_scenario_creation_task(self, agent):
        return Task(description=dedent(f"""
            <task>
                Generate detailed, realistic and relevant threat scenarios from the 
                context information about the system:
            </task>
    ...
    def control_measure_task(self, agent):

        return Task(description=dedent(f"""
            <task>
                Generate a list of well defined control or countermeasures 
                from the threat scenarios and other context information provided.
            </task>
                                       
                   
            <description>
                - Review information about the system.
                - Review the mermaid diagram representation of the system architecture.
                - Review the trust zones for each node.
                - Review the threat scenarios
                - Review any provided organization controls and policies and incorporate 
                the relevant ones into the countermeasures.

                - Using all the preceeding information, generate one or more countermeasure 
                that can be implemented to mitigate the threat. Annotate the control with a 
                reference of Policy, Standard or framework on which the suggested controls 
                are ground. For example PCI Requirement 5, NIST 800-53, OWASP etc.
