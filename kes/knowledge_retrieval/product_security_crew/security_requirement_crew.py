...
class SecurityRequirementCrew:
    ...
    def run(self, system_information, image_path, main_model, vision_model):
        ...
        if image_path:
            arch_diagram_information = tasks.architecture_image_analysis_task(
                image_path=image_path, agent=agents.system_information_agent()
            )
        ...
        threat_scenario_task = tasks.threat_scenario_creation_task(
            system_description=output.raw, agent=agents.threat_scenario_agent()
        )
        ...
        requirement_task = tasks.requirement_generation_task(
            system_description=output.raw, agent=agents.requirement_agent()
        )
    ...
    def execute(self, main_model, system_information):
        ...
        if system_information:
            requirement_task = tasks.requirement_generation_task(
                system_description=system_information, agent=agents.requirement_agent()
            )
